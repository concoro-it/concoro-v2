...
export default async function QuestaSettimanaPage() {
    ...
    const [{ data: concorsi, count }, tier] = await Promise.all([
        getScadenzaQuestaSettimana(supabase, 1, 50),
    ...
    return (
        <div className="container max-w-container mx-auto px-4 py-8">
            <nav className="flex items-center gap-1.5 text-sm text-muted-foreground mb-6">
                ...
                <ChevronRight className="w-4 h-4" />
                <span className="text-foreground">Questa Settimana</span>
            </nav>

            <div className="mb-8">
                <h1 className="text-3xl font-bold tracking-tight">
                    📅 Concorsi in scadenza questa settimana
                    <span className="ml-3 text-lg font-normal text-muted-foreground">({count})</span>
                </h1>
                <p className="text-muted-foreground mt-1">
                    {count} concorsi scadono nei prossimi 7 giorni.
                </p>
            </div>
            ...
            {count === 0 && (
                <div className="text-center py-12 border rounded-xl bg-muted/30">
                    <p className="text-muted-foreground">Nessun concorso in scadenza questa settimana.</p>
                </div>
            )}
        </div>
    );
